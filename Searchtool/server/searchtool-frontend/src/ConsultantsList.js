...
const getEmails = (c) => (c.contacts?.emails?.length ? c.contacts.emails.map(e => e.value) : c.emails) || [];
const getPhones = (c) => (c.contacts?.phones?.length ? c.contacts.phones.map(p => p.value) : c.phones) || [];
...
export default function ConsultantsList() {
  ...
  const [view, setView] = useState(params.get('view') || 'list');     // default list

  useEffect(() => {
    const API = process.env.REACT_APP_API_URL || 'http://localhost:8081';
    setLoading(true);
    axios.get(`${API}/api/consultants`)
      .then(res => { setRows(res.data || []); setErr(''); })
      .catch(e => { console.error(e); setErr('Failed to load consultants'); })
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((c) => {
      // first search: by consultant basics
      const basicsMatch =
        q1 === '' ||
      ...
      // experience search (only active when first search has text)
      const expMatch =
        q2 === '' ||
        toArr(c.experience).some(e =>
      ...
      const expSelOk =
        expertise === 'All' ||
        toArr(c.expertise).includes(expertise);

      if (!expSelOk) return false;

      // qualification dropdown
      const qualOk =
        qual === 'All' || toArr(c.qualifications).includes(qual);
  ...
  const sorted = useMemo(() => {
    const arr = [...filtered];
    const [field, dir] = sort.split('-'); // name / updated / rating  and asc/desc
  ...
  return (
    <div className="page">
      {/* top bar with back buttons + auth */}
  ...
  <div className="cell exp">{/* <- add exp class here */}
              ...
              <table className="consultants-table">
                <thead>
                  <tr>
                    ...
                    {user?.role === 'admin' && <th className="th-actions">Actions</th>}
                  </tr>
                </thead>
                <tbody>
                  {sorted.length === 0 ? (
                    <tr><td colSpan={user?.role === 'admin' ? 6 : 5} className="no-results">No consultants found.</td></tr>
                  ) : sorted.map((c) => {
                    ...
                    return (
                      <tr key={c._id || c.id || c.name}>
                        ...
                        <td>{exps}</td>
                        <td>
                          <div className="stars" aria-label={`${ravg} stars`}>
                        ...
                        {user?.role === 'admin' && (
                          <td className="cell-actions">
                            <button className="btn-small" onClick={() => navigate(`/consultants/${c._id}${qs}`)}>Edit</button>
                            <button className="btn-small danger" onClick={() => handleDelete(c._id)}>Delete</button>
                          </td>
                        )}
                      </tr>
                    );
